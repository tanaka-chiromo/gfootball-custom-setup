...
class CustomBuild(build_ext):
  ...
  def run_unix(self):
    ...
    if use_prebuilt_lib:
      ...
      # Compile the engine with the same interpreter pip is using.
      env_prefix = 'PYTHON="%s" ' % sys.executable.replace('"', '\\"')
      if os.system(env_prefix + 'gfootball/build_game_engine.sh'):
...
setup(
    name='gfootball',
    version='2.10.3.post1',
    description=('Google Research Football (tournament build) - patched env '
                 'that installs on modern Python without the PyPI gym pin'),
    long_description=open(os.path.join(_HERE, 'README.md'), encoding='utf-8').read(),
    long_description_content_type='text/markdown',
    author='Google LLC (tournament patches by tanaka-chiromo)',
    author_email='no-reply@google.com',
    url='https://github.com/tanaka-chiromo/gfootball-custom-setup',
    license='Apache 2.0',
    python_requires='>=3.9',
    packages=packages,
